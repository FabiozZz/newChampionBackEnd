...
const SelectHeadersGroup = ({label,value,setValue,data,placeholder='не выбрано',...props}) => {
    /**
            ...
            <div ref={optionBox} className={classes.optionBox}>
                {(data&&data.length)?data.map(item=> (
                    <React.Fragment key={item.id}>
                        <p className={classes.label_group}>{item.name}</p>
                        {item.groups.map(group=>(
                            <option key={group.id} onClick={()=> {
                                setValue(group);
                                setToggleOptionBox(false);
                            }} value={group.name} className={classes.item}>{group.name}</option>
                        ))}
                    </React.Fragment>
                    ))
                    :null}
...
export default SelectHeadersGroup;