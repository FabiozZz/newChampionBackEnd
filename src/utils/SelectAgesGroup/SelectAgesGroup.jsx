...
const SelectAgesGroup = ({label='возрастная группа',value,setValue,data={},danger=false,placeholder='не выбрано',disabled=false,...props}) => {
    /**
    ...
    // },[data]);
    const classWrapper = disabled?classes.disabled:danger ? classes.danger : classes.inputWrapper;
    return (
        <div ref={selectRef} className={`${classes.selectBoxWrapper} ${props.className}`} {...props}>
            <label className={`${classes.label}`} >{label}</label>
            <div className={classWrapper} onClick={handleToggleOptionBox}>
                ...
                <span/>
                <input className={classes.input} value={value.label} placeholder={placeholder} disabled={true} type="text" />
                {toggleOptionBox&&<div className={classes.arrowOptionBox}/>}
            </div>
            {toggleOptionBox&&
            <div ref={optionBox} className={classes.optionBox}>
                {(data&&data.length)?data.map(item=> (<option key={item.id} onClick={()=> {
                    setValue(item);
                    setToggleOptionBox(false);
                }} value={item.name} className={classes.item}>{item.label}</option>))
                :null}
            </div>
            }
        </div>
...
export default SelectAgesGroup;