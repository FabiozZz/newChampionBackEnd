            ...
            <>
                {dataUri?
                    ...
                    <>
                        ...
                        <Button text={'Загрузить фото с устройства'} click={()=>inputForDevices.current.click()} size={"auto"}/>
                            <input ref={inputForDevices} type="file" hidden={true} onChange={handleTakePhoto} accept="image/*;capture=camera"/>
                        </div>
                    </>
                }
            </>
                        ...
                        <EditCameraContext.Provider value={{dataUri, takePhoto: handleTakePhotoAnimationDone}}>
                            {cameraOn === 'off'?
                                <>
                                    ...
                                    <div className={classes.btn_wrappewr}>
                                        <Button text={'Загрузить фото с устройства'} click={()=>inputForDesctop.current.click()} size={"auto"}/>
                                        <input ref={inputForDesctop} onChange={(e)=>setDataUri(e.target.files[0])} accept={'image/*'} type="file" hidden={true}/>
                                        <Button text={'Сделать фото'} click={changePhotoModOn} factor={'success'} size={"auto"}/>
                                    </div>
                                </>
                                :cameraOn === 'on'?
                                    <CameraOn />
