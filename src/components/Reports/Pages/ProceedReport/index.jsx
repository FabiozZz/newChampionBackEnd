...
const ProceedReport = () => {
	...
	let filter_fake_data = [];
	for (const chips in chips_type_pay) {
		if (chips_type_pay[chips].id === 0 && chips_type_pay[chips].active) {
			let temp_data = Array(...new Set(fake_data.map(_ => _.pay_info.type)));
			console.log(temp_data);
			let temp_fiter = temp_data.map(_ => ({
				[_]: fake_data.filter(data => data.pay_info.type === _),
				color: getRandomColor(),
			}));
			filter_fake_data = temp_fiter;
			break;
		}

		if (chips.active) {
			filter_fake_data.push(fake_data.map(_ => _.pay_info.type === chips.type));
		}
	}
	console.log('filter_fake_data', filter_fake_data);
	console.log(chips_type_pay);
	...
	const getData = canvas => {
		...
		for (const keyI in filter_fake_data) {
			for (const keyK in filter_fake_data[keyI]) {
				if (typeof filter_fake_data[keyI][keyK] === 'object') {
				}
			}
		}
		...
		// 	datasets: filter_fake_data.map(_ => ({
		// 		data: _.map(e => Number(e.pay_info.total.replace(/\s/gi, ''))),
		// 		fill: true,
	...
	const lineChart = <Line type={'line'} data={getData} options={options_line} />;
	...
	const left_chart = [
		...
		{ id: 2, name: 'Разовые тренировки', value: '6 980', color: getRandomColor() },
		{ id: 3, name: 'Персональные тренировки', value: '12 914', color: getRandomColor() },
	];
	const right_chart = [
		{ id: 1, name: 'Наличными', value: '125 569', color: getRandomColor() },
		{ id: 2, name: 'Безналичными', value: '987 463', color: getRandomColor() },
	];
						...
						<div className={cn(classes.chart_container, 'gcol-md-12')}>
							...
							<div className="gcol-md-12 container-g">
								...
								<div className={'gcol-md-6 flex flex-column '}>
									{left_chart.map(_ => (
										<div key={_.id} className={'flex align-center gap-10'}>
											<div
												style={{
													width: 8,
													height: 8,
							...
							<div className={'gcol-md-12 flex flex-flex-start gap-16'}>
								{/*{types_pay.map(_ => (*/}
								{/*	<>*/}
								{/*		<Chips active={}></Chips>*/}
								{/*	</>*/}
