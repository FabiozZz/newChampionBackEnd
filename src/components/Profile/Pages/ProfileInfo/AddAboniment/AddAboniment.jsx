	...
	useEffect(() => {
		if (selectAboniment.prices && selectStatus.id) {
			let price = selectAboniment.prices.find(
				item =>
					item.age_group.id === user.club_card.age_group.id &&
					item.level.id === selectStatus.id
			).price;
			if (price) {
			...
			{selectAboniment.name !== '' && selectStatus.name !== '' ? (
				<>
					<h3 className={classes.block_info__title_aboniment}>
						{selectAboniment.name} для {selectStatus.name.replace(/[а-я]{2}$/gi, 'ых')}{' '}
						клиентов
					</h3>
					<div className={classes.add_aboniment}>
						<div className={classes.sales_card}>
							...
							<div className={`${classes.sale_count}`}>
								<span className={`${classes.sale_count_text}`}>
									{selectAboniment.train_quantity > 20 ? (
								...
								<span className={`${classes.sale_count_text}`}>
									{selectAboniment.days_duration / 7 > 8 ? (
										<span dangerouslySetInnerHTML={{ __html: '&#8734;' }} />
									) : (
										selectAboniment.days_duration / 7
									)}{' '}
									{declOfWeekNum(selectAboniment.days_duration / 7)}
								</span>
