 ...
 * @constructor
 */
export const SelectGroup = ({required=false,name,value,setValue,label,data={},disabled=false,...props}) => {
    ...
    return (
        <div ref={selectRef} className={`${classes.selectBoxWrapper} ${props.className}`} {...props}>
            ...
            <div className={disabledClass} onClick={!disabled&&handleToggleOptionBox}>
                ...
                <span/>
                <input readOnly={true} required={required} ref={refInput} name={name} className={classes.input} value={value.name||''} onClick={handleChangeValue} disabled={disabled} type="text" placeholder={props.placeholder||'Не выбрано'}/>
