...
export const Option = ({ change, id, couch, couchList }) => {
	...
	useEffect(() => {
		if (optionBox.current != null) {
			optionBox.current.style.bottom =
				-optionBox.current.getBoundingClientRect().height + 'px';
	...
	return (
		<div
			onClick={handleToggleOptionBox}
			ref={selectRef}
			className={classes.block_trainer}>
							...
							if (item.id !== 0) {
								return (
									<option
										...
										className={classes.optionBox_item}>
										{trainer}
									</option>
								);
							} else {
