...
/**
 * Компонент изменения вида списка клиентов
 *
		...
		<div className={classes.sort_section}>
			<div className={classes.sort_section__sort}>
				<span className={classes.text_muted}>
					Показано:{' '}
					<span className={classes.text_result}>
						{clients?.length || 0} {declOfResults(clients?.length || 0)}
					</span>
				</span>
				<span className={classes.text_muted}>
					Сортровать по: <SortSelect />
				</span>
			</div>
			<div className={classes.sort_section_factor}>
				<svg
					onClick={row}
					className={cn(
						{ [classes.factor_active]: !active },
						{ [classes.factor_rows]: active }
					)}
					width="24"
					height="24"
					viewBox="0 0 24 24"
