...
export const SettingsAbonement = (children, func) => {
    ...
    const removeAbonement = (ids) => {
        dispatch(remove_item_abonement(ids))
    };
        ...
        <>
            ...
            <div className={classes.general}>
                ...
                <div className={classes.wrapper}>
                    {abonements ?
                        <>
                            <h4 className={classes.label}>Активные тарифы</h4>
                            <table cellSpacing={4} cellPadding={18} className={classes.table}>
                                ...
                                <tbody>
                                {!!abonements.length&&abonements.sort((a,b)=>a.id<b.id?-1:1).filter(abonement=>!abonement.in_archive).map(item => {
                                    const mouseMove = (e) => {
                ...
                {!!abonements.length&&!!abonements.filter(abonement=>abonement.in_archive).length&&
                <div className={classes.wrapper}>
                    <h4 className={classes.label}>Архивные тарифы</h4>
                    <table cellSpacing={4} cellPadding={18} className={classes.table}>
                        <thead>
                        <tr>
                            <th>название</th>
                        ...
                        <tbody>
                        {!!abonements.length&&abonements.filter(abonement=>abonement.in_archive).map(item => {
                            const mouseMove = (e) => {
                                if (e.target.getAttribute('class') === classes.left) {
                                    for (let i = 0; i < e.currentTarget.children.length; i++) {
                                        let item = e.currentTarget.children[i];
