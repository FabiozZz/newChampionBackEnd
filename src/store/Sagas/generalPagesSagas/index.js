...
export function* loggedInApp({ payload }) {
	try {
		...
		yield put(
			log_in_done({ type: 'success', title: 'Поздравляю', desc: 'Вы вошли в систему' })
		);
		yield put(load_general_page_data());
	} catch (e) {
...
export function* addedDebtClient({ payload }) {
	console.log(payload);
	try {
		yield call(() => Api.createDebtTrain(payload));
		yield put(load_general_page_data());
	} catch (e) {
		yield put(load_general_page_data_fail([e.message]));
	}
...
export function* addedOnceClient({ payload }) {
	console.log(payload);
	try {
		yield call(() => Api.createOnceTrain(payload));
		yield put(load_general_page_data());
	} catch (e) {
		yield put(load_general_page_data_fail([e.message]));
	}
...
export function* buyAndAdded({ payload }) {
	const { abonement, client } = payload;
